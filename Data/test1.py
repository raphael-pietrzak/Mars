...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if not hovered and building_image is not None:
                x, y = pygame.mouse.get_pos()
                building_sprite = Sprite(x - building_image.get_width() / 2, y - building_image.get_height() / 2, building_image)
                sprites.add(building_sprite)
    ...
    if not hovered and building_image is not None:
        x, y = pygame.mouse.get_pos()
        screen.blit(building_image, (x - building_image.get_width() / 2, y - building_image.get_height() / 2))
